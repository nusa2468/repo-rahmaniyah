...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans antialiased text-slate-800">
        ...
        <div>
            <nav class="flex mb-3">
                <ol class="inline-flex items-center space-x-1 md:space-x-2 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 italic">
                    ...
                    <li><i class="fas fa-chevron-right text-[7px] opacity-50 mx-2"></i></li>
                    <li class="text-slate-600 italic">SETTING INDIVIDU</li>
                </ol>
            </nav>
            <h1 class="text-3xl font-black tracking-tighter text-slate-900 dark:text-white uppercase italic leading-none">
                Detail Gaji <span class="text-indigo-600"><?= esc($pegawai['nama_lengkap']) ?></span>
            </h1>
        </div>

        <a href="<?= base_url('app/kepegawaian/gaji-pegawai') ?>" class="inline-flex items-center px-6 py-3 bg-white border-2 border-slate-200 text-slate-700 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:border-indigo-600 hover:text-indigo-600 transition-all shadow-sm active:scale-95">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div class="bg-white dark:bg-slate-800 p-6 rounded-[2.5rem] border-2 border-slate-100 dark:border-white/5 shadow-xl">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 text-xl shadow-inner"><i class="fas fa-user-tie"></i></div>
                <div>
                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Status Pegawai</p>
                    <p class="text-sm font-black text-slate-800 dark:text-white uppercase italic"><?= esc($pegawai['jenis_pegawai']) ?> (Unit <?= esc($pegawai['kode_jenjang']) ?>)</p>
                </div>
            </div>
            <div class="mt-4 pt-4 border-t border-slate-100 flex justify-between items-center">
                <?php
                    $nomorInduk = !empty($pegawai['nip']) ? $pegawai['nip'] : (!empty($pegawai['nipy']) ? $pegawai['nipy'] : ($pegawai['nik'] ?? '-'));
                    $labelInduk = !empty($pegawai['nip']) ? 'NIP' : (!empty($pegawai['nipy']) ? 'NIPY' : 'NIK');
                ?>
                <span class="text-[10px] font-bold text-slate-500 uppercase tracking-wide"><?= $labelInduk ?>: <span class="text-slate-900 dark:text-white"><?= esc($nomorInduk) ?></span></span>
                <span class="px-2 py-0.5 bg-emerald-50 text-emerald-600 text-[9px] font-black rounded uppercase">AKTIF</span>
            </div>
        </div>

        <div class="md:col-span-2 bg-slate-900 p-6 rounded-[2.5rem] shadow-xl text-white relative overflow-hidden group">
            <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h2 class="text-4xl font-black italic">Rp <?= number_format($total_pendapatan - $total_potongan, 0, ',', '.') ?></h2>
                </div>
                <div class="flex gap-6 text-right">
                    <div>
                        <p class="text-[9px] font-bold text-emerald-400 uppercase tracking-widest mb-1">Total Pendapatan</p>
                        <p class="text-lg font-black text-emerald-100">Rp <?= number_format($total_pendapatan, 0, ',', '.') ?></p>
                    </div>
                    <div>
                        <p class="text-[9px] font-bold text-rose-400 uppercase tracking-widest mb-1">Total Potongan</p>
                        <p class="text-lg font-black text-rose-100">Rp <?= number_format($total_potongan, 0, ',', '.') ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-sm border-2 border-slate-100 dark:border-white/5 overflow-hidden sticky top-24">
                <div class="bg-slate-50 dark:bg-white/5 px-8 py-6 border-b border-slate-100 dark:border-white/10">
                    <h3 class="text-xs font-black text-slate-800 dark:text-white uppercase tracking-widest italic leading-none">
                        <i class="fas fa-plus-circle mr-2 text-indigo-600"></i> Tambah Komponen
                    </h3>
                </div>
                <form action="<?= base_url('app/kepegawaian/gaji-pegawai/simpanKomponen') ?>" method="post" class="p-8 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select name="id_komponen" required class="w-full pl-4 pr-10 py-3 bg-white border-2 border-slate-200 rounded-xl text-xs font-bold uppercase focus:border-indigo-500 outline-none appearance-none cursor-pointer">
                                <option value="">-- PILIH KOMPONEN --</option>
                                <?php foreach($master_komponen as $m): ?>
                                    <option value="<?= $m['id'] ?>" data-nominal="<?= $m['nominal_default'] ?>">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="jumlah" id="nominal_input" required placeholder="0" 
                               class="w-full px-4 py-3 bg-white border-2 border-slate-200 rounded-xl text-sm font-black focus:border-indigo-500 outline-none">
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-[11px]">
                            <?php if(empty($list_gaji)): ?>
                                <tr><td colspan="4" class="px-8 py-20 text-center opacity-30 italic font-bold">Belum ada komponen gaji yang diatur.</td></tr>
                            <?php else: ?>
                                <?php foreach($list_gaji as $row): ?>
                                    <tr class="hover:bg-slate-50 transition-colors group">
                                        <td class="px-8 py-5 font-black text-slate-700 uppercase"><?= esc($row['nama_komponen']) ?></td>
                                        <td class="px-6 py-5 text-center">
                                            <span class="px-3 py-1 rounded-full text-[9px] font-black uppercase border <?= $row['tipe_komponen'] == 1 ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100' ?>">
                                                <?= $row['tipe_komponen'] == 1 ? 'PENDAPATAN' : 'POTONGAN' ?>
                                            </span>
                                        </td>
                                        <td class="px-6 py-5 text-right font-black <?= $row['tipe_komponen'] == 1 ? 'text-emerald-600' : 'text-rose-500' ?>">
                                            Rp <?= number_format($row['jumlah_set'], 0, ',', '.') ?>
                                        </td>
                                        <td class="px-8 py-5 text-center">
                                            <a href="<?= base_url('app/kepegawaian/gaji-pegawai/hapusKomponen/' . $row['id']) ?>" onclick="return confirm('Hapus komponen ini?')" 
                                               class="w-8 h-8 inline-flex items-center justify-center bg-white border-2 border-slate-200 rounded-xl text-rose-500 hover:bg-rose-50 hover:border-rose-200 transition-all shadow-sm">
...
<script>
    ...
    document.querySelector('select[name="id_komponen"]').addEventListener('change', function() {
        ...
        if(nominal) document.getElementById('nominal_input').value = nominal;
    });
</script>
