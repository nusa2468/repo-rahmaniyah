...
<div class="px-4 sm:px-6 py-6 max-w-7xl mx-auto space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-900 dark:text-white tracking-tight uppercase italic flex items-center gap-3">
                <i class="fas fa-balance-scale text-amber-500"></i> Laporan Posisi Keuangan
            </h1>
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">
                Laporan Neraca sesuai standar akuntansi yayasan (ISAK 35).
            </p>
        </div>
    ...
    <!-- GLOBAL NAVIGATION TABS (DENGAN DROPDOWN LAPORAN) -->
    <div class="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl w-fit flex-wrap shadow-inner no-print z-40 relative">
        ...
        <div x-data="{ openLaporan: false }" class="relative">
            ...
            <div x-show="openLaporan" x-transition.opacity.duration.200ms style="display: none;" class="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 py-2 z-[100] flex flex-col">
                <div class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest">Pilih Jenis Laporan</div>
                <a href="<?= base_url('app/akuntansi/laporan/posisi-keuangan') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 hover:text-amber-600 transition-colors flex items-center">
                    <i class="fas fa-balance-scale w-6 text-center text-amber-500 mr-2"></i> Neraca (Posisi Keuangan)
                </a>
                ...
                <a href="<?= base_url('app/akuntansi/laporan/arus-kas') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 hover:text-amber-600 transition-colors flex items-center">
                    <i class="fas fa-water w-6 text-center text-amber-500 mr-2"></i> Arus Kas
                </a>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border border-slate-200 dark:border-slate-800 p-6 md:p-8 no-print relative z-10">
        <form action="" method="get" class="flex flex-col md:flex-row gap-6 items-end">
            ...
            <div class="space-y-2 flex-grow">
                ...
                <div class="relative">
                    <select name="jenjang" onchange="this.form.submit()" class="w-full pl-4 pr-10 py-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 uppercase appearance-none cursor-pointer focus:ring-2 focus:ring-amber-500 outline-none transition-colors hover:border-amber-300">
                        <option value="MULTI" <?= $filterJenjang === 'MULTI' ? 'selected' : '' ?> class="font-black text-amber-600">📊 KONSOLIDASI MULTI-KOLOM (Pusat & Semua Cabang)</option>
            ...
            <!-- Periode Akhir -->
            <div class="space-y-2">
                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Posisi Per Tanggal</label>
            ...
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-lg hover:shadow-xl active:scale-95 border-b-4 border-orange-700">Tampilkan</button>
        </form>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-[2rem] shadow-xl border border-slate-200 dark:border-slate-800 p-8 md:p-12 overflow-hidden print-container relative z-0">
        ...
        <div class="text-center mb-8">
            <h2 class="text-lg md:text-xl font-black text-slate-900 dark:text-white uppercase tracking-widest underline decoration-2 underline-offset-4">LAPORAN POSISI KEUANGAN (NERACA)</h2>
            <?php 
            ...
            <p class="text-[10px] font-black text-indigo-500 mt-1 uppercase tracking-widest bg-indigo-50 dark:bg-indigo-900/20 inline-block px-3 py-1 rounded-md border border-indigo-100 dark:border-indigo-800">
                Posisi Per Tanggal: <?= date('d F Y', strtotime($endDate)) ?>
            </p>
        </div>

        <?php if($isMultiColumn): ?>
            <!-- ========================================================================= -->
            <!-- TAMPILAN: KONSOLIDASI MULTI-KOLOM (EXCEL STYLE) -->
            <!-- ========================================================================= -->
                ...
                <table class="w-full text-left text-sm whitespace-nowrap border-collapse min-w-[800px] print-table">
                    ...
                    <tbody class="text-slate-800 dark:text-slate-300">
                        ...
                        <tr class="bg-slate-100 dark:bg-slate-800/80 border-b border-slate-300 dark:border-slate-700 print-gray-row">
                            <td colspan="100%" class="p-4 font-black text-slate-800 dark:text-white uppercase tracking-widest">ASET (HARTA)</td>
                        </tr>
                        <?php foreach($laporan['Aset'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50 dark:bg-slate-800/50 print-gray-row' : 'hover:bg-amber-50 dark:hover:bg-slate-800/50' ?>">
                                <td class="p-3 border-r border-slate-100 dark:border-slate-800 <?= $isHeader ? 'font-bold uppercase text-xs pl-4' : 'pl-8 text-slate-600 dark:text-slate-400' ?>">
                        ...
                        <tr class="font-black bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border-t-2 border-b-4 border-double border-emerald-300 dark:border-emerald-800 print-strong-row">
                            <td class="p-4 uppercase tracking-widest border-r border-emerald-200 dark:border-emerald-700">Jumlah Aset</td>
                            <?php foreach($activeUnits as $uKode => $uName): ?>
                                <td class="p-4 text-right text-xs border-r border-emerald-200 dark:border-emerald-700"><?= number_format($totalPerUnit['Aset'][$uKode], 0, ',', '.') ?></td>
                            <?php endforeach; ?>
                            <td class="p-4 text-right bg-emerald-100 dark:bg-emerald-900/40 text-base print:bg-transparent">
                        ...
                        <tr class="bg-slate-100 dark:bg-slate-800/80 border-b border-slate-300 dark:border-slate-700 mt-4 print-gray-row">
                            <td colspan="100%" class="p-4 font-black text-slate-800 dark:text-white uppercase tracking-widest">LIABILITAS (KEWAJIBAN)</td>
                        </tr>
                        <?php foreach($laporan['Liabilitas'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50 dark:bg-slate-800/50 print-gray-row' : 'hover:bg-amber-50 dark:hover:bg-slate-800/50' ?>">
                                <td class="p-3 border-r border-slate-100 dark:border-slate-800 <?= $isHeader ? 'font-bold uppercase text-xs pl-4' : 'pl-8 text-slate-600 dark:text-slate-400' ?>">
                                    <?= $isHeader ? '<i class="fas fa-folder text-amber-500 mr-2"></i>' : '<i class="fas fa-angle-right text-slate-300 mr-2"></i>' ?>
                        ...
                        <tr class="font-black bg-rose-50 dark:bg-rose-900/20 text-rose-700 dark:text-rose-400 border-t-2 border-rose-200 dark:border-rose-800 print-strong-row">
                            <td class="p-4 uppercase tracking-widest text-xs border-r border-rose-200 dark:border-rose-700">Jumlah Liabilitas</td>
                            <?php foreach($activeUnits as $uKode => $uName): ?>
                                <td class="p-4 text-right text-xs border-r border-rose-200 dark:border-rose-700"><?= number_format($totalPerUnit['Liabilitas'][$uKode], 0, ',', '.') ?></td>
                            <?php endforeach; ?>
                        ...
                        <!-- 3. ASET NETO (EKUITAS) -->
                        <tr class="bg-slate-100 dark:bg-slate-800/80 border-b border-slate-300 dark:border-slate-700 print-gray-row">
                            <td colspan="100%" class="p-4 font-black text-slate-800 dark:text-white uppercase tracking-widest">ASET NETO (EKUITAS)</td>
                        </tr>
                        <?php foreach($laporan['Aset Neto'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50 dark:bg-slate-800/50 print-gray-row' : 'hover:bg-amber-50 dark:hover:bg-slate-800/50' ?>">
                                <td class="p-3 border-r border-slate-100 dark:border-slate-800 <?= $isHeader ? 'font-bold uppercase text-xs pl-4' : 'pl-8 text-slate-600 dark:text-slate-400' ?>">
                                    <?= $isHeader ? '<i class="fas fa-folder text-amber-500 mr-2"></i>' : '<i class="fas fa-angle-right text-slate-300 mr-2"></i>' ?>
                        ...
                        <!-- TOTAL ASET NETO -->
                        <tr class="font-black bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border-t-2 border-amber-200 dark:border-amber-800 print-strong-row">
                            <td class="p-4 uppercase tracking-widest text-xs border-r border-amber-200 dark:border-amber-700">Jumlah Aset Neto</td>
                            <?php foreach($activeUnits as $uKode => $uName): ?>
                                <td class="p-4 text-right text-xs border-r border-amber-200 dark:border-amber-700"><?= number_format($totalPerUnit['Aset Neto'][$uKode], 0, ',', '.') ?></td>
                            <?php endforeach; ?>
                            <td class="p-4 text-right bg-amber-100 dark:bg-amber-900/40 text-sm print:bg-transparent">
                        ...
                        <tr class="font-black bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 border-y-4 border-double border-indigo-300 dark:border-indigo-800 print-grand-total">
                            <td class="p-5 uppercase tracking-widest border-r border-indigo-200 dark:border-indigo-700">JUMLAH LIABILITAS & ASET NETO</td>
                            <?php foreach($activeUnits as $uKode => $uName): ?>
                                <td class="p-5 text-right text-sm border-r border-indigo-200 dark:border-indigo-700"><?= number_format($totalPerUnit['Liabilitas'][$uKode] + $totalPerUnit['Aset Neto'][$uKode], 0, ',', '.') ?></td>
                            <?php endforeach; ?>
                            <td class="p-5 text-right bg-indigo-100 dark:bg-indigo-900/40 text-base print:bg-transparent">
                                <?= number_format($totalLiabilitas + $totalAsetNeto, 0, ',', '.') ?>
                            </td>
                        </tr>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap border-collapse print-table">
                    <tbody class="text-slate-800 dark:text-slate-300">
                        ...
                        <tr class="border-b-2 border-slate-800 dark:border-slate-400 print-strong-row bg-slate-50 dark:bg-slate-800/80">
                            <td colspan="2" class="py-3 px-4 font-black text-slate-900 dark:text-white uppercase tracking-widest text-sm">ASET (HARTA)</td>
                        </tr>
                        <?php foreach ($laporan['Aset'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50/50 dark:bg-slate-800/30' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50' ?>">
                                <td class="py-2.5 <?= $isHeader ? 'font-bold uppercase text-xs pl-6 text-slate-700' : 'pl-10 font-medium text-slate-600' ?>">
                                    <?= esc($akun['nama_akun']) ?>
                                </td>
                        ...
                        <tr class="font-black bg-slate-50 dark:bg-slate-800/50 border-t border-b-4 border-slate-800 dark:border-slate-500 print-gray-row text-emerald-700 dark:text-emerald-400">
                            <td class="py-3 pl-4 uppercase tracking-widest text-xs">Total Aset</td>
                            <td class="py-3 pr-4 text-right">Rp <?= number_format($totalAset, 0, ',', '.') ?></td>
                        </tr>
                        ...
                        <tr class="border-b-2 border-slate-800 dark:border-slate-400 mt-8 print-strong-row bg-slate-50 dark:bg-slate-800/80">
                            <td colspan="2" class="py-3 px-4 font-black text-slate-900 dark:text-white uppercase tracking-widest text-sm">LIABILITAS (KEWAJIBAN)</td>
                        </tr>
                        <?php foreach ($laporan['Liabilitas'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50/50 dark:bg-slate-800/30' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50' ?>">
                        ...
                        <tr class="font-black bg-slate-50 dark:bg-slate-800/50 border-t border-slate-800 dark:border-slate-500 print-gray-row text-rose-700 dark:text-rose-400">
                            <td class="py-3 pl-4 uppercase tracking-widest text-xs">Total Liabilitas</td>
                            <td class="py-3 pr-4 text-right">Rp <?= number_format($totalLiabilitas, 0, ',', '.') ?></td>
                        </tr>
                        ...
                        <!-- 3. ASET NETO -->
                        <tr class="border-b-2 border-slate-800 dark:border-slate-400 mt-8 print-strong-row bg-slate-50 dark:bg-slate-800/80">
                            <td colspan="2" class="py-3 px-4 font-black text-slate-900 dark:text-white uppercase tracking-widest text-sm">ASET NETO (EKUITAS)</td>
                        </tr>
                        <?php foreach ($laporan['Aset Neto'] as $akun): ?>
                            <?php $isHeader = $akun['is_parent'] == 1; ?>
                            <tr class="border-b border-slate-100 dark:border-slate-800 transition-colors <?= $isHeader ? 'bg-slate-50/50 dark:bg-slate-800/30' : 'hover:bg-slate-50 dark:hover:bg-slate-800/50' ?>">
                        ...
                        <?php endforeach; ?>
                        <tr class="font-black bg-slate-50 dark:bg-slate-800/50 border-t border-slate-800 dark:border-slate-500 print-gray-row text-amber-700 dark:text-amber-400">
                            <td class="py-3 pl-4 uppercase tracking-widest text-xs">Total Aset Neto</td>
                            <td class="py-3 pr-4 text-right">Rp <?= number_format($totalAsetNeto, 0, ',', '.') ?></td>
                        </tr>
                        ...
                        <tr class="font-black text-lg text-slate-900 dark:text-white border-y-4 border-double border-slate-900 dark:border-slate-400 print-grand-total mt-4">
                            <td class="py-5 pl-4 uppercase tracking-widest">TOTAL LIABILITAS & ASET NETO</td>
                            <td class="py-5 pr-4 text-right text-indigo-700 dark:text-indigo-400">
                                Rp <?= number_format($totalLiabilitas + $totalAsetNeto, 0, ',', '.') ?>
                            </td>
                        </tr>
                    </tbody>
                </table>
        ...
        <!-- NOTIFIKASI BALANCE -->
        <?php if ($totalAset !== ($totalLiabilitas + $totalAsetNeto)): ?>
            <div class="bg-rose-50 dark:bg-rose-900/20 text-center py-4 border-t border-rose-200 dark:border-rose-800 text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-widest animate-pulse no-print mt-6 rounded-b-2xl">
                <i class="fas fa-exclamation-triangle mr-2"></i> Peringatan: Neraca Tidak Seimbang (Terdapat Selisih)
            </div>
        <?php else: ?>
            <div class="bg-emerald-50 dark:bg-emerald-900/20 text-center py-4 border-t border-emerald-200 dark:border-emerald-800 text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest no-print mt-6 rounded-b-2xl">
                <i class="fas fa-check-circle mr-2"></i> Neraca Seimbang (Balanced)
            </div>
        <?php endif; ?>
...
<style>
    @media print {
        ...
        /* Pewarnaan Tabel saat Cetak */
        .print-table { border-collapse: collapse !important; color: black !important; }
