...
<div x-data="{ activeTab: '<?= esc($activeTabFromUrl) ?>', deleteModalOpen: false, deleteActionUrl: '', deleteItemName: '' }" class="px-4 sm:px-6 py-6 max-w-7xl mx-auto space-y-8">
    ...
    <!-- GLOBAL NAVIGATION TABS (MENU BAWAH) -->
    <div class="flex items-center gap-2 p-1.5 bg-slate-100 dark:bg-slate-900 rounded-2xl w-fit flex-wrap shadow-inner no-print z-40 relative">
        <button @click="activeTab = 'dashboard'" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all" :class="activeTab == 'dashboard' ? 'bg-white text-amber-600 shadow-md' : 'text-slate-500 hover:text-amber-600 hover:bg-white/50'">
            <i class="fas fa-home mr-2"></i> Beranda
        </button>
        <button @click="activeTab = 'coa'" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all" :class="activeTab == 'coa' ? 'bg-white text-amber-600 shadow-md' : 'text-slate-500 hover:text-amber-600 hover:bg-white/50'">
            <i class="fas fa-sitemap mr-2"></i> Master COA
        </button>
        <a href="<?= base_url('app/akuntansi/jurnal') ?>" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-slate-500 hover:text-amber-600 hover:bg-white/50 whitespace-nowrap">
            <i class="fas fa-book mr-2"></i> Jurnal Umum
        </a>
        <a href="<?= base_url('app/akuntansi/buku-besar') ?>" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all text-slate-500 hover:text-amber-600 hover:bg-white/50 whitespace-nowrap">
            <i class="fas fa-book-open mr-2"></i> Buku Besar
        </a>

        <!-- DROPDOWN KHUSUS LAPORAN KEUANGAN -->
        <div x-data="{ openLaporan: false }" class="relative">
            <button @click="openLaporan = !openLaporan" @click.away="openLaporan = false" class="px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 text-slate-500 hover:text-amber-600 hover:bg-white/50">
            ...
            <!-- Menu Dropdown -->
            <div x-show="openLaporan" x-cloak x-transition.opacity.duration.200ms class="absolute top-full left-0 mt-2 w-56 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-100 dark:border-slate-700 py-2 z-[100] flex flex-col">
                <div class="px-4 py-2 text-[9px] font-black text-slate-400 uppercase tracking-widest">Sesuai Standar ISAK 35</div>
                <a href="<?= base_url('app/akuntansi/laporan/neraca-saldo') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
                    <i class="fas fa-list-ol w-6 text-center text-amber-500 mr-2"></i> Neraca Saldo (Trial)
                </a>
                <a href="<?= base_url('app/akuntansi/laporan/posisi-keuangan') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
                    <i class="fas fa-balance-scale w-6 text-center text-amber-500 mr-2"></i> Posisi Keuangan (Neraca)
                </a>
                <a href="<?= base_url('app/akuntansi/laporan/aktivitas') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
                    <i class="fas fa-chart-line w-6 text-center text-amber-500 mr-2"></i> Penghasilan Komprehensif
                </a>
                <a href="<?= base_url('app/akuntansi/laporan/perubahan-aset-neto') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
                    <i class="fas fa-layer-group w-6 text-center text-amber-500 mr-2"></i> Perubahan Aset Neto
                </a>
                <a href="<?= base_url('app/akuntansi/laporan/arus-kas') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
                    <i class="fas fa-water w-6 text-center text-amber-500 mr-2"></i> Laporan Arus Kas
                </a>
                <div class="border-t border-slate-100 dark:border-slate-700 my-1"></div>
                <a href="<?= base_url('app/akuntansi/laporan/calk') ?>" class="px-4 py-2.5 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-amber-50 dark:hover:bg-amber-900/20 hover:text-amber-600 dark:hover:text-amber-400 transition-colors flex items-center">
