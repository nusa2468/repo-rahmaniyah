...
<div class="max-w-7xl mx-auto space-y-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 font-medium text-slate-800 dark:text-white md:ml-2">Kurikulum</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-black text-gray-800 dark:text-white tracking-tight">Master Kurikulum</h1>
            <div class="flex items-center gap-2 mt-1">
                <span class="w-2 h-2 rounded-full bg-indigo-500"></span>
                <p class="text-sm text-gray-500 dark:text-gray-400">Standar Kompetensi & Materi Pendidikan</p>
                <!-- Indikator Role & Unit -->
        ...
        <div class="flex flex-wrap items-center gap-2 h-10">
            ...
            <!-- Tombol Tambah (h-10) -->
            <a href="<?= base_url('app/masterdata/kurikulum/new') ?>" 
               ...
               style="height: 40px !important;">
                ...
                <span class="hidden sm:inline pt-0.5">Kurikulum Baru</span>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="bg-indigo-600 rounded-2xl p-5 shadow-lg shadow-indigo-500/30 relative overflow-hidden group">
            ...
            <p class="text-xs font-bold text-indigo-100 uppercase tracking-wider relative z-10">Total Kurikulum</p>
            <div class="flex items-baseline gap-1 mt-1 relative z-10">
                <h3 class="text-3xl font-black text-white"><?= $pager->getTotal('kurikulum') ?></h3>
                <span class="text-xs text-indigo-200 font-medium">Data</span>
            </div>
            <i class="fas fa-scroll absolute bottom-4 right-4 text-white/20 text-3xl"></i>
        </div>
        ...
        <div class="bg-emerald-600 rounded-2xl p-5 shadow-lg shadow-emerald-500/30 relative overflow-hidden group">
            ...
            <div class="flex items-baseline gap-1 mt-1 relative z-10">
                <?php 
                    // Hitung aktif di halaman ini untuk indikator cepat
                    $activeOnPage = 0;
                    foreach($kurikulumData as $k) { if(strtolower($k['status'] ?? '') == 'aktif') $activeOnPage++; }
                ?>
        ...
        <!-- Non-Aktif / Arsip -->
        <div class="bg-slate-600 rounded-2xl p-5 shadow-lg shadow-slate-500/30 relative overflow-hidden group">
            <div class="absolute right-0 top-0 w-24 h-24 bg-white/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110"></div>
            <p class="text-xs font-bold text-slate-100 uppercase tracking-wider relative z-10">Non-Aktif</p>
            <div class="flex items-baseline gap-1 mt-1 relative z-10">
                 <?php 
                    $inactiveOnPage = count($kurikulumData) - $activeOnPage;
                ?>
                 <h3 class="text-3xl font-black text-white"><?= $inactiveOnPage ?></h3>
                 <span class="text-xs text-slate-300 font-medium">Arsip Halaman</span>
            </div>
            <i class="fas fa-archive absolute bottom-4 right-4 text-white/20 text-3xl"></i>
        </div>
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="border-b border-gray-200 dark:border-white/10 p-4 bg-gray-50/50 dark:bg-white/5">
            <form action="" method="get" class="flex flex-col sm:flex-row gap-2 w-full md:w-auto items-center justify-between">
                ...
                <div class="relative w-full md:w-96">
                    ...
                    <input type="text" name="search" value="<?= esc($search) ?>" placeholder="Cari Kurikulum / Kode..." class="w-full pl-9 pr-4 py-2.5 text-sm bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none">
                </div>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
                <thead class="bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold uppercase text-[10px] tracking-wider border-b border-gray-200 dark:border-white/10">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center w-16">ID & Unit</th> <!-- KOLOM ID & UNIT DIGABUNG -->
                        <!-- KOLOM UNIT DIHILANGKAN -->
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    <?php 
                    $pageStart = ($current_filter['per_page'] * ($pager->getCurrentPage('kurikulum') - 1)) + 1;
                    ...
                    <?php else: foreach ($kurikulumData as $idx => $row): $row = (array)$row; ?>
                        <tr class="hover:bg-indigo-50/30 dark:hover:bg-white/5 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-center">
                                <div class="flex flex-col items-center gap-1.5">
                                    ...
                                    <!-- Unit Badge (Hanya Kode Jenjang) -->
                                    <span class="inline-flex px-1.5 py-0.5 rounded text-[9px] font-black border shadow-sm uppercase tracking-wide <?= getJenjangBadge($row['kode_jenjang']) ?>">
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="text-[10px] text-gray-400 mt-0.5 font-mono">Kode: <?= esc($row['kode_kurikulum']) ?></div>
                            </td>

                            <td class="px-6 py-4">
                                <div class="text-xs text-gray-500 max-w-xs truncate" title="<?= esc($row['deskripsi']) ?>">
                                    <?= esc($row['deskripsi'] ?: '-') ?>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?= base_url('app/masterdata/kurikulum/edit/' . $row['id']) ?>" 
                                    ...
                                    <form action="<?= base_url('app/masterdata/kurikulum/delete/' . $row['id']) ?>" method="post" class="contents" onsubmit="return confirm('Hapus Kurikulum <?= esc($row['nama_kurikulum']) ?>? Data yang terhubung mungkin akan terdampak.')">
                                        <?= csrf_field() ?>
        ...
        <div class="px-6 py-4 border-t border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-gray-800/30">
            <?= $pager->links('kurikulum', 'tailwind_pagination') ?>
        </div>
    </div>
</div>
<?= $this->endSection() ?>