...
<div class="w-full flex flex-col gap-8">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-[10px] font-black uppercase tracking-[0.3em] text-sky-600">Content Manager</span>
            </div>
            <h1 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight">
                Berita & <span class="text-transparent bg-clip-text bg-gradient-to-r from-sky-600 to-indigo-600">Artikel</span>
            </h1>
        </div>
        <a href="<?= base_url('app/cms/berita/new') ?>" class="inline-flex items-center px-6 py-3 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black uppercase tracking-widest rounded-xl shadow-lg shadow-sky-500/30 transition-all transform hover:scale-105">
    ...
    <?php
        // --- LOGIKA UI SCOPE & FILTER (Sync dengan Controller) ---
        ...
        // Label Unit Pengguna
        $userLabel = $isGlobal ? 'GLOBAL / YAYASAN' : ($listUnit[$sessUnit] ?? $sessUnit);
    ?>

    <!-- 3. INFO BAR & FILTER (SCOPE UNIT) -->
        ...
        <div class="bg-white dark:bg-gray-800 p-5 border-l-4 border-gray-300 dark:border-gray-600 shadow-md flex items-center justify-center rounded-2xl">
            <div class="text-center">
                ...
                <h3 class="text-xl font-black text-gray-800 dark:text-white mt-1 italic leading-none">
                    <?= !empty($currentFilter) ? esc($currentFilter) : 'SEMUA DATA' ?>
                </h3>
            </div>
        </div>
    </div>

    <!-- 4. STATISTIK KPI (PHP Logic Calculation) -->
    <?php
    $total = count($berita);
    $sd = 0; $smp = 0; $sma = 0; $global = 0;
    foreach ($berita as $b) {
        // Menggunakan array access
        $j = $b['jenjang'] ?? $b['kode_jenjang'] ?? 'Global'; 
        if ($j == 'SD') $sd++;
        if ($j == 'SMP') $smp++;
        if ($j == 'SMA') $sma++;
    ...
    $cards = [
        ['label' => 'Total Berita', 'val' => $total, 'bg' => 'bg-indigo-500', 'border' => 'border-indigo-700', 'icon' => 'newspaper'],
    ...
    <!-- 5. CONTENT GRID (Table & Chart) -->
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        <!-- Tabel Berita (Kiri - Span 2) -->
            ...
            <div class="bg-white dark:bg-gray-900 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-gray-50 dark:border-white/5 flex items-center justify-between">
                    <h3 class="text-lg font-black text-gray-900 dark:text-white flex items-center gap-3">
                        <i class="fas fa-list text-sky-500"></i> Daftar Berita
                    </h3>
                </div>
                    ...
                    <table class="w-full text-left">
                        <thead class="bg-gray-50/50 dark:bg-white/5">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Judul & Info</th>
                            ...
                            <?php if (!empty($berita)) : ?>
                                <?php $no = 1; foreach ($berita as $item) : ?>
                                    <tr class="hover:bg-sky-50/30 dark:hover:bg-white/5 transition-colors group">
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="font-bold text-gray-800 dark:text-gray-200 mb-1 line-clamp-1 group-hover:text-sky-600 transition-colors">
                                                <?= esc($item['judul']) ?>
                                            </div>
                                            <div class="flex items-center gap-3 text-[10px] text-gray-400 font-medium uppercase tracking-wider">
                                                <span class="flex items-center gap-1">
                                                    <i class="fas fa-user-circle text-sky-400"></i> <?= esc($item['penulis'] ?? 'Admin') ?>
                                                </span>
                                        ...
                                        <td class="px-6 py-4 text-right">
                                            <div class="flex items-center justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                                                <a href="<?= base_url('app/cms/berita/edit/' . $item['id']) ?>" 
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-12 text-center text-gray-400">
                                        <div class="flex flex-col items-center justify-center gap-2">
                                            <i class="fas fa-newspaper text-4xl opacity-20"></i>
                                            <span class="text-xs font-bold uppercase tracking-wider">Belum ada berita dipublikasikan</span>
                                        </div>
                                    </td>
                                </tr>
                            <?php endif; ?>
            ...
            <div class="bg-white dark:bg-gray-900 p-6 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
                ...
                <div class="mt-6 flex flex-wrap justify-center gap-3">
                    <div class="flex items-center gap-1.5">
                        ...
                        <span class="text-[10px] font-bold text-gray-500 uppercase">SD</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                        <span class="text-[10px] font-bold text-gray-500 uppercase">SMP</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-sky-500"></span>
                        <span class="text-[10px] font-bold text-gray-500 uppercase">SMA</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <span class="w-2 h-2 rounded-full bg-amber-500"></span>
                        <span class="text-[10px] font-bold text-gray-500 uppercase">Global</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-indigo-600 to-violet-700 p-6 rounded-3xl shadow-lg shadow-indigo-500/20 text-white relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h4 class="font-black text-sm uppercase tracking-wider mb-1">Info Cepat</h4>
                            <p class="text-xs text-indigo-100 leading-relaxed font-medium">
                                Berita berlabel <strong>GLOBAL</strong> akan otomatis muncul di halaman utama yayasan dan seluruh sub-domain unit sekolah.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white px-6 pt-6 pb-4 sm:p-8">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-black text-gray-900 uppercase italic tracking-tight">Hapus Berita?</h3>
                        <div class="mt-2">
                            <p class="text-xs text-gray-500 leading-relaxed">
                                Apakah Anda yakin ingin menghapus berita ini secara permanen? Data yang dihapus tidak dapat dipulihkan kembali.
                            </p>
                        </div>
                    </div>
                </div>
...
<script>
    // Konfigurasi Chart
    const ctx = document.getElementById('newsDistributionChart').getContext('2d');
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: ['#f43f5e', '#10b981', '#0ea5e9', '#f59e0b'], // Rose, Emerald, Sky, Amber
                borderWidth: 0,
                hoverOffset: 10
            }],
        },
    ...
    function confirmDelete(id) {
        ...
        btn.href = '<?= base_url('app/cms/berita/delete/') ?>/' + id;
