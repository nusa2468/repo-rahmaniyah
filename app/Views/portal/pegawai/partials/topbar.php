<!-- Mobile Topbar / Nav Toggle (Emerald Theme for Staff) -->
<div class="lg:hidden fixed top-0 left-0 w-full z-50 bg-emerald-600 text-white shadow-lg overflow-hidden transition-all duration-300">
    ...
    <div class="absolute bottom-0 left-0 w-24 h-24 bg-emerald-400/20 rounded-full mix-blend-overlay filter blur-xl translate-y-1/2 -translate-x-1/2 pointer-events-none"></div>
    ...
    <div class="relative z-10 px-4 py-3 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3 text-lg font-bold truncate max-w-[75%]">
            ...
            <div class="flex flex-col justify-center overflow-hidden">
                <span class="truncate leading-tight"><?= esc($sekolah->nama_sekolah ?? 'Portal Pegawai') ?></span>
                <?php if(isset($pegawai['nama_lengkap'])): ?>
                    <span class="text-[10px] font-normal text-emerald-100 truncate leading-tight">
                        Hi, <?= esc(strtok($pegawai['nama_lengkap'], " ")) ?> 👨‍🏫
                    </span>
                <?php endif; ?>
            </div>
        </div>
