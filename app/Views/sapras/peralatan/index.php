...
<div class="px-4 py-8 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans antialiased text-slate-900">
        ...
        <div>
            ...
            <nav class="flex mt-1" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                    ...
                    <li aria-current="page"><div class="flex items-center text-amber-600"><i class="fas fa-chevron-right mx-2 text-[8px]"></i> <span class="uppercase italic underline decoration-2">Peralatan</span></div></li>
                </ol>
            </nav>
        </div>
        <div class="flex items-center gap-2">
            <a href="<?= base_url('app/sapras/peralatan/new') ?>" class="inline-flex items-center px-4 py-2 text-xs font-black uppercase tracking-widest bg-amber-500 text-white hover:bg-amber-600 transition-all shadow-lg border-b-4 border-amber-700"><i class="fas fa-plus mr-2 text-[10px]"></i> Tambah Alat</a>
        </div>
    </div>
    ...
    <div class="border-b border-slate-200 mb-8 overflow-x-auto">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            ...
            <a href="<?= base_url('app/sapras/peralatan') ?>" class="border-amber-500 text-amber-600 whitespace-nowrap py-4 px-1 border-b-2 font-black text-xs uppercase tracking-widest"><i class="fas fa-tools mr-2"></i> Peralatan</a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="md:col-span-2 bg-slate-800 p-5 border-b-4 <?= $amIUnitAdmin ? 'border-amber-500' : 'border-emerald-500' ?> shadow-lg flex flex-col sm:flex-row items-center justify-between relative overflow-hidden group">
            ...
            <div class="flex items-center gap-5 z-10 w-full">
                <div class="w-12 h-12 <?= $amIUnitAdmin ? 'bg-amber-500' : 'bg-emerald-500' ?> flex items-center justify-center text-white shadow-lg rounded-sm"><i class="fas <?= $amIUnitAdmin ? 'fa-lock' : 'fa-globe-asia' ?> text-xl"></i></div>
                <div class="flex-1">
                    <p class="text-[10px] font-black <?= $amIUnitAdmin ? 'text-amber-300' : 'text-emerald-300' ?> uppercase tracking-widest leading-none mb-1"><?= $amIUnitAdmin ? 'MODE AKSES TERBATAS' : 'MODE SUPERADMIN / YAYASAN' ?></p>
        ...
        <div class="bg-amber-500 p-5 border-b-4 border-amber-700 shadow-lg flex items-center justify-between relative overflow-hidden">
            <div class="absolute -bottom-4 -right-4 text-amber-400 opacity-30"><i class="fas fa-boxes text-8xl"></i></div>
            <div class="z-10">
                <p class="text-[10px] font-black text-amber-100 uppercase tracking-widest leading-none mb-1">Total Unit Barang</p>
                <h3 class="text-3xl font-black text-white"><?php $totalItem = 0; foreach($peralatan as $p) { $totalItem += (int)($p['jumlah'] ?? 0); } echo number_format($totalItem, 0, ',', '.'); ?></h3>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border-t-4 border-slate-800 shadow-2xl overflow-hidden mb-6">
        <div class="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-[11px] font-black text-slate-700 uppercase tracking-widest italic">Data Inventaris Peralatan</h2>
            </div>
            <?php if(!empty($currentFilter)): ?><span class="px-2 py-1 bg-indigo-100 text-indigo-700 text-[9px] font-bold uppercase rounded-sm border border-indigo-200">Filter Aktif: <?= esc($currentFilter) ?></span><?php endif; ?>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-100 border-b-2 border-slate-200 uppercase">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200">Nama Barang</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200 text-center">Kondisi</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200 text-center">Jumlah</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    ...
                    <?php else : ?>
                        ...
                        foreach ($peralatan as $p) : ?>
                            <tr class="hover:bg-amber-50/30 transition-colors group border-b border-slate-50">
                                ...
                                <td class="px-6 py-4 border-r border-slate-50">
                                    <div class="text-[11px] font-black text-slate-800 uppercase italic leading-none mb-1"><?= esc($p['nama']) ?></div>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-sky-100 text-sky-700"><?= esc($p['kode_jenjang'] ?? 'GLOBAL') ?></span>
                                </td>
                                <td class="px-6 py-4 border-r border-slate-50 text-center"><span class="px-2 py-1 bg-white border border-slate-200 text-[9px] font-bold rounded-sm uppercase"><?= esc($p['kondisi']) ?></span></td>
                                <td class="px-6 py-4 border-r border-slate-50 text-center"><?= esc($p['jumlah']) ?></td>
                                <td class="px-6 py-4 border-r border-slate-50 max-w-[200px]"><p class="text-[10px] font-medium text-slate-500 truncate italic"><?= esc($p['keterangan']) ?></p></td>
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="<?= base_url('app/sapras/peralatan/edit/' . $p['id']) ?>" class="w-7 h-7 flex items-center justify-center bg-white border border-slate-200 text-amber-500 hover:bg-amber-500 hover:text-white shadow-sm rounded-sm"><i class="fas fa-pencil-alt text-[10px]"></i></a>
                                        <button onclick="confirmDelete('<?= $p['id'] ?>', '<?= esc($p['nama']) ?>')" class="w-7 h-7 flex items-center justify-center bg-white border border-slate-200 text-rose-500 hover:bg-rose-500 hover:text-white shadow-sm rounded-sm"><i class="fas fa-trash-alt text-[10px]"></i></button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <?php if (!empty($peralatan)) : ?><div class="bg-slate-50 px-6 py-4 border-t border-slate-200"><?= $pager->links('default', 'tailwind_pagination') ?></div><?php endif; ?>
    </div>
</div>
...
<script>
    function confirmDelete(id, nama) { document.getElementById('modal-delete-info').innerText = nama; document.getElementById('deleteBtn').href = `<?= base_url('app/sapras/peralatan/delete/') ?>` + id; document.getElementById('deleteModal').classList.remove('hidden'); }
