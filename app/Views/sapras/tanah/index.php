...
<div class="px-4 py-8 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans antialiased text-slate-900">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3 text-[10px] font-bold uppercase tracking-widest text-slate-500">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center text-indigo-600">
                            ...
                            <span class="uppercase italic underline decoration-2">Tanah</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="flex items-center gap-2">
            <a href="<?= base_url('app/sapras/tanah/new') ?>" 
               class="inline-flex items-center px-4 py-2 text-xs font-black uppercase tracking-widest bg-indigo-600 text-white hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200 active:scale-95 border-b-4 border-indigo-800">
                <i class="fas fa-plus mr-2 text-[10px]"></i> Tambah Aset
            </a>
        </div>
    </div>

    <!-- FITUR NAVIGASI TAB SAPRAS -->
    <div class="border-b border-slate-200 mb-8 overflow-x-auto">
        <nav class="-mb-px flex space-x-8" aria-label="Tabs">
            <a href="<?= base_url('app/sapras/tanah') ?>" class="border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-black text-xs uppercase tracking-widest">
            ...
            <a href="<?= base_url('app/sapras/ruangan') ?>" class="border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 whitespace-nowrap py-4 px-1 border-b-2 font-bold text-xs uppercase tracking-widest transition-all">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div class="md:col-span-2 bg-slate-800 p-5 border-b-4 <?= $amIUnitAdmin ? 'border-indigo-500' : 'border-emerald-500' ?> shadow-lg flex flex-col sm:flex-row items-center justify-between relative overflow-hidden group">
            <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                <i class="fas <?= $amIUnitAdmin ? 'fa-building-lock' : 'fa-globe' ?> text-9xl text-white transform rotate-12"></i>
            </div>
            
            <div class="flex items-center gap-5 z-10 w-full">
                <div class="w-12 h-12 <?= $amIUnitAdmin ? 'bg-indigo-500' : 'bg-emerald-500' ?> flex items-center justify-center text-white shadow-lg rounded-sm">
                    <i class="fas <?= $amIUnitAdmin ? 'fa-lock' : 'fa-globe-asia' ?> text-xl"></i>
                </div>
                <div class="flex-1">
                    <p class="text-[10px] font-black <?= $amIUnitAdmin ? 'text-indigo-300' : 'text-emerald-300' ?> uppercase tracking-widest leading-none mb-1">
                        <?= $amIUnitAdmin ? 'MODE AKSES TERBATAS' : 'MODE SUPERADMIN / YAYASAN' ?>
                    </p>
                    <h3 class="text-xl font-black text-white uppercase italic leading-none tracking-tight">
                        <?= $amIUnitAdmin ? 'UNIT: ' . esc($sessUnit) : 'AKSES SELURUH UNIT' ?>
                    </h3>
                </div>
        ...
        <!-- Kartu Total -->
        <div class="bg-indigo-600 p-5 border-b-4 border-indigo-800 shadow-lg flex items-center justify-between relative overflow-hidden">
            <div class="absolute -bottom-4 -right-4 text-indigo-500 opacity-30">
                <i class="fas fa-layer-group text-8xl"></i>
            </div>
            <div class="z-10">
                <p class="text-[10px] font-black text-indigo-200 uppercase tracking-widest leading-none mb-1">Total Aset Tanah</p>
                <div class="flex items-baseline gap-2">
                    <h3 class="text-3xl font-black text-white"><?= count($tanah) ?></h3>
                    <span class="text-[10px] text-indigo-100 font-bold uppercase italic">Persil</span>
                </div>
            </div>
            <div class="z-10 bg-indigo-500/30 p-2 rounded-sm backdrop-blur-sm">
    ...
    <div class="bg-white border-t-4 border-slate-800 shadow-2xl overflow-hidden mb-6">
        <div class="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <h2 class="text-[11px] font-black text-slate-700 uppercase tracking-widest italic">Data Inventaris Tanah</h2>
            </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-100 border-b-2 border-slate-200 uppercase">
                        ...
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200">Identitas</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200 text-right">Luas</th>
                        <th class="px-6 py-4 text-[10px] font-black text-slate-500 tracking-widest border-r border-slate-200">Legalitas</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    <?php if (empty($tanah)) : ?>
                        <tr>
                            <td colspan="6" class="px-6 py-20 text-center bg-slate-50/50">
                                <p class="text-xs font-black text-slate-400 uppercase tracking-widest italic">Data Kosong</p>
                            </td>
                        </tr>
                    <?php else : ?>
                        <?php 
                        ...
                        foreach ($tanah as $t) : ?>
                            <tr class="hover:bg-indigo-50/50 transition-colors group border-b border-slate-50">
                                ...
                                <td class="px-6 py-4 border-r border-slate-50">
                                    <div class="text-[11px] font-black text-slate-800 uppercase italic leading-none mb-1"><?= esc($t['nama']) ?></div>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider bg-slate-100 text-slate-600">
                                        <?= esc($t['kode_jenjang'] ?? 'GLOBAL') ?>
                                    </span>
                                </td>
                                <td class="px-6 py-4 border-r border-slate-50 text-right">
                                    <div class="text-[12px] font-black text-slate-700 tracking-tighter italic leading-none"><?= number_format($t['luas'], 2, ',', '.') ?></div>
                                    <div class="text-[9px] font-bold text-slate-400 uppercase mt-0.5">m²</div>
                                </td>
                                <td class="px-6 py-4 border-r border-slate-50 whitespace-nowrap">
                                    <span class="text-[10px] font-bold text-slate-600 uppercase tracking-tight font-mono"><?= esc($t['sertifikat']) ?></span>
                                </td>
                                <td class="px-6 py-4 border-r border-slate-50 max-w-[250px]">
                                    <p class="text-[10px] font-medium text-slate-500 leading-snug truncate italic"><?= esc($t['keterangan']) ?></p>
                                </td>
                                <td class="px-6 py-4 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="<?= base_url('app/sapras/tanah/edit/' . $t['id']) ?>" class="w-7 h-7 flex items-center justify-center bg-amber-500 text-white shadow-sm rounded-sm"><i class="fas fa-pencil-alt text-[10px]"></i></a>
                                        <button onclick="confirmDelete('<?= $t['id'] ?>', '<?= esc($t['nama']) ?>')" class="w-7 h-7 flex items-center justify-center bg-rose-500 text-white shadow-sm rounded-sm"><i class="fas fa-trash-alt text-[10px]"></i></button>
                                    </div>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0 font-sans">
        ...
        <div class="inline-block align-bottom bg-white rounded-none text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full border-t-8 border-rose-600">
            <div class="bg-white px-6 pt-6 pb-4 sm:p-8 sm:pb-6">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-sm font-black text-slate-900 uppercase tracking-widest italic" id="modal-title">Konfirmasi Hapus</h3>
                        <div class="mt-3 bg-slate-50 p-4 border-l-4 border-slate-300">
                            <p class="text-[11px] font-bold text-slate-500 uppercase leading-relaxed">
                                Hapus aset: <span id="modal-delete-info" class="block text-slate-900 mt-1 font-black underline italic tracking-tight uppercase"></span>
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-slate-50 px-6 py-4 sm:px-8 sm:flex sm:flex-row-reverse gap-2">
                <a id="deleteBtn" href="#" class="w-full inline-flex justify-center px-6 py-2 bg-rose-600 text-[10px] font-black text-white uppercase tracking-widest hover:bg-rose-700 shadow-lg shadow-rose-200 transition-all border-b-4 border-rose-800">Hapus</a>
...
<script>
    function confirmDelete(id, nama) {
        ...
        document.getElementById('deleteBtn').href = `<?= base_url('app/sapras/tanah/delete/') ?>` + id;
