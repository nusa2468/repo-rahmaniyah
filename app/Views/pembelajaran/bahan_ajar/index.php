...
<div class="container mx-auto px-4 py-8 space-y-6">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <span class="ml-1 font-medium text-slate-800 dark:text-white md:ml-2">Bahan Ajar</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-4 border-b border-gray-200 dark:border-gray-700 pb-6">
        <div>
            <h1 class="text-3xl font-black text-gray-800 dark:text-white tracking-tight">Manajemen Bahan Ajar</h1>
            <div class="flex items-center gap-2 mt-2">
                ...
                <p class="text-sm text-gray-500">Bank Materi & Sumber Belajar Digital</p>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2 w-full lg:w-auto">
            ...
            <!-- 2. Tombol Generate Bank Soal Massal -->
            <form action="<?= base_url('app/pembelajaran/bank-soal/generate_massal') ?>" method="post" class="flex-grow lg:flex-none" onsubmit="return confirm('Sistem akan membuat draf Bank Soal untuk setiap Bahan Ajar yang belum memiliki soal. Lanjutkan?');">
                <?= csrf_field() ?>
            ...
            <a href="<?= base_url('app/pembelajaran/bahan-ajar/new') ?>" class="h-10 flex-grow lg:flex-none inline-flex justify-center items-center px-4 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-lg shadow-md hover:shadow-lg hover:shadow-indigo-500/30 transition-all transform active:scale-95 gap-2">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-indigo-600 rounded-2xl p-5 shadow-lg shadow-indigo-500/30 relative overflow-hidden group">
            ...
            <p class="text-xs font-bold text-indigo-100 uppercase tracking-wider relative z-10">Total Materi</p>
        ...
        <div class="bg-rose-600 rounded-2xl p-5 shadow-lg shadow-rose-500/30 relative overflow-hidden group">
            <div class="absolute right-0 top-0 w-24 h-24 bg-white/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110"></div>
            <p class="text-xs font-bold text-rose-100 uppercase tracking-wider relative z-10">Dokumen PDF</p>
            <p class="text-3xl font-black text-white mt-1 relative z-10"><?= number_format($stats['pdf'] ?? 0) ?></p>
        </div>
        
        <!-- Video (Blue/Sky) -->
        <div class="bg-sky-600 rounded-2xl p-5 shadow-lg shadow-sky-500/30 relative overflow-hidden group">
            <div class="absolute right-0 top-0 w-24 h-24 bg-white/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110"></div>
            <p class="text-xs font-bold text-sky-100 uppercase tracking-wider relative z-10">Video Tutorial</p>
            <p class="text-3xl font-black text-white mt-1 relative z-10"><?= number_format($stats['video'] ?? 0) ?></p>
        </div>
        
        <!-- PPT (Amber/Orange) -->
        <div class="bg-amber-500 rounded-2xl p-5 shadow-lg shadow-amber-500/30 relative overflow-hidden group">
            <div class="absolute right-0 top-0 w-24 h-24 bg-white/10 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-110"></div>
            <p class="text-xs font-bold text-amber-100 uppercase tracking-wider relative z-10">Presentasi</p>
            <p class="text-3xl font-black text-white mt-1 relative z-10"><?= number_format($stats['ppt'] ?? 0) ?></p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        <form action="" method="get" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="keyword" id="keyword" value="<?= esc($filter_keyword) ?>" placeholder="Cari Judul Materi..." class="block w-full h-10 pl-10 rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 sm:text-sm p-2.5 border focus:ring-indigo-500 focus:border-indigo-500">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Judul & Unit</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-widest">Format</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Referensi RPP</th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-500 uppercase tracking-widest">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    <?php if(!empty($bahan_ajar)): ?>
                        <?php foreach ($bahan_ajar as $item): ?>
                            ...
                            <tr class="hover:bg-indigo-50/30 dark:hover:bg-indigo-900/20 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-bold text-gray-900 dark:text-gray-100"><?= esc($item['judul_bahan']) ?></div>
                                    <div class="flex items-center gap-2 mt-1">
                                        <span class="text-[10px] font-bold text-gray-500 bg-gray-100 px-2 py-0.5 rounded border border-gray-200">
                                            <?= $item['kode_jenjang'] ?>
                                        </span>
                                        <span class="text-[10px] text-gray-400">Mapel ID: <?= $item['mata_pelajaran_id'] ?></span>
                                    </div>
                                    <div class="text-xs text-gray-500 italic mt-1 truncate max-w-xs"><?= esc($item['deskripsi']) ?></div>
                                </td>
                                
                                <td class="px-6 py-4 text-center">
                                    <?php 
                                        $fType = $item['jenis_file'];
                                        $style = match($fType) {
                                        ...
                                        $icon = match($fType) {
                                            'PDF' => 'fa-file-pdf', 'Video' => 'fa-play-circle', 'PPT' => 'fa-file-powerpoint', default => 'fa-file'
                                        };
                                    ?>
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-black border uppercase <?= $style ?>">
                                        <i class="fas <?= $icon ?>"></i> <?= $fType ?>
                                    </span>
                                </td>
                                
                                <td class="px-6 py-4">
                                    <?php if(!empty($item['rpp_topik'])): ?>
                                        <div class="flex flex-col">
                                            <span class="text-xs font-semibold text-indigo-600 dark:text-indigo-400">
                                                <?= esc($item['rpp_topik']) ?>
                                            </span>
                                            <span class="text-[10px] text-gray-400 mt-0.5">Terhubung RPP</span>
                                        </div>
                                    <?php else: ?>
                                        <span class="text-[10px] text-gray-400 italic bg-gray-50 px-2 py-1 rounded border border-dashed border-gray-300">
                                            Belum terhubung RPP
                                        </span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                    <div class="flex justify-center gap-1">
                                        <!-- Download -->
                                        <a href="<?= base_url($item['file_path']) ?>" target="_blank" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Download File">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                        </a>

                                        <!-- Edit -->
                                        <a href="<?= base_url('app/pembelajaran/bahan-ajar/edit/' . $item['id']) ?>" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Edit Data">
                                        ...
                                        <!-- Hapus -->
                                        <form action="<?= base_url('app/pembelajaran/bahan-ajar/delete/' . $item['id']) ?>" method="post" class="inline-block" onsubmit="return confirm('Hapus materi ini?');">
                                            <?= csrf_field() ?>
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center">
                                    <svg class="w-12 h-12 text-gray-200 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4"></path></svg>
                                    <p class="text-gray-500 font-medium">Belum ada bahan ajar yang diunggah.</p>
                                </div>
                            </td>
                        </tr>
                    <?php endif; ?>
    ...
    <!-- Pagination -->
    <div class="mt-4">
        <?= $pager->links('bahan', 'tailwind_pagination') ?>
    </div>
</div>
<?= $this->endSection() ?>